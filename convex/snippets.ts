...
export const starSnippet = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // toggles star, if already, unstar, else star
...
export const deleteComment = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (comment.userId !== identity.subject) throw new ConvexError("You can delete your comment only!");
...
export const deleteSnippet = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const isCreator = snippet.userId === identity.subject;
    if (!isCreator) throw new ConvexError("You can't delete others' snippets!");

    // A snippets can have comments and stars
    const comments = await ctx.db
      .query("snippetComments")
      .withIndex("by_snippet_id")
      .filter((q) => q.eq(q.field("snippetId"), args.snippetId))
